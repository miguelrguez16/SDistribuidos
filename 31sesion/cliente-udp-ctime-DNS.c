...
int main(int argc, char * argv[]){
    ...
    //compilar y probar con ./cliente-DNS time-a-g.nist.gov 
    if (argc != 2) {
	...
	char *ip = inet_ntoa( ( struct in_addr )*( (struct in_addr *) ( hp -> h_addr_list[0]) ));
    


	socket_datos = socket(PF_INET, SOCK_DGRAM, 0);
	dir_servidor.sin_family = AF_INET;
	dir_servidor.sin_addr.s_addr = inet_addr(ip);//dir_servidor.sin_addr.s_addr = inet_addr("129.6.15.28");
